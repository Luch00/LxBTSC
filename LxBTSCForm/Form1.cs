...
namespace LxBTSCForm
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            //browser.LoadHtml(HTML, "file:///chat");
            LoadHtml();
        }
        ...
        private static void InitCef()
        {
            ...
            if (!Cef.IsInitialized)
            {
                ...
                CefSettings set = new CefSettings
                {
                    ...
                    //BrowserSubprocessPath = @"C:\CefLib\CefSharp.BrowserSubprocess.exe",
                    //BrowserSubprocessPath = @"plugins\CefLib\CefSharp.BrowserSubprocess.exe",
                    BrowserSubprocessPath = Path.Combine(appdataPath, @"Luch\LxBTSC\CefLib", "CefSharp.BrowserSubprocess.exe"),
                    //LocalesDirPath = @"plugins\CefLib\locales",
                    LocalesDirPath = Path.Combine(appdataPath, @"Luch\LxBTSC\CefLib\locales"),
                    //LocalesDirPath = @"C:\CefLib\locales",
                    //ResourcesDirPath = @"plugins\CefLib"
                    ResourcesDirPath = Path.Combine(appdataPath, @"Luch\LxBTSC\CefLib")
                    //ResourcesDirPath = @"C:\CefLib",
                };
...
//</html>";
    ...
    class DeviceChangeNotifier : Form
    {
        ...
        private static void WinEventProc(IntPtr hWinEventHook, uint eventType, IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            ...
            //if (eventType == (uint)WinEvent.EVENT_OBJECT_SELECTION)
            //{
            //    //browser.ExecuteScriptAsync($"AddLine('<div>Event SELECTION</div>');");

            //    // check open channel
            //}
            //if (eventType == (uint)WinEvent.EVENT_OBJECT_LOCATIONCHANGE && idObject == 0 && idChild == 0)
            //{
            //    browser.ExecuteScriptAsync($"AddLine('<div>Event LOCATION</div>');");
            //    //check rect position
            //}
            //if (eventType != EVENT_OBJECT_LOCATIONCHANGE && eventType != 288 && eventType != 286 && eventType != 287 && eventType != 32780 && eventType != (uint)WinEvent.EVENT_OBJECT_SELECTION)
            //{
            //    Debug.WriteLine("Event: " + hwnd.ToInt32());
            //    browser.ExecuteScriptAsync($"AddLine('<div>Event {(WinEvent)eventType} {idObject} {idChild}</div>');");
            //}
        }
